...
fun part1(input: List<String>): Long {
    ...
    // Step 3: Repeat this process until we arrive at the right day.
...
fun retrieveAmountOfFishes(days: Int, input: List<String>) : Long{
    ...
    val entries = input.first().split(",").map { it.toInt() }.groupingBy { it }.eachCount();

    for (entry in entries)
        fishes[entry.key] = entry.value.toLong();
    ...
    for (day in 0 until days){
        ...
        println("Day $day with lantern fish size ${fishes.values.sum()}")
    }
